...
def listagem_cliente(request):
        if request.POST and request.POST['imput_pesquisa']:
            dados = Cliente.objects.filter(nome__contains=request.POST['imput_pesquisa'])
        else:
            dados = Cliente.objects.all()
        contexto = {'dados': dados, 'txt':'Digite o nome do cliente'}
